...
import {
  ...
  Pizza, Beef, Fish, Coffee, Cake, UtensilsCrossed,
...
const seedPublic = [
  { id: 1, name: "Hambúrgueria do Beto", category: "Hamburgueria", bairro: "Centro", score: 9.2, count: 143,
    address: "Rua dos Andradas, 540 - Centro", phone: "(51) 3225-4410", instagram: "@hamburgueriadobeto",
    tags: ["Bom para ir em grupo", "Atendimento rápido"],
    reviews: [
      { name: "Ana", score: 10, comment: "Melhor smash burger da cidade, sem dúvida." },
      { name: "Rafael", score: 9, comment: "Ambiente simples, mas o lanche compensa." },
    ] },
  { id: 2, name: "Sushi Kaito", category: "Japonês", bairro: "Moinhos", score: 8.7, count: 89,
    address: "Rua Padre Chagas, 88 - Moinhos de Vento", phone: "(51) 3346-2290", instagram: "@sushikaito",
    reviews: [
      { name: "Camila", score: 9, comment: "Peixe muito fresco, voltarei sempre." },
      { name: "Diego", score: 8, comment: "Preço justo pela qualidade." },
    ] },
  { id: 3, name: "Pizzaria Napoli", category: "Pizzaria", bairro: "Floresta", score: 9.5, count: 210,
    address: "Av. Cristóvão Colombo, 1200 - Floresta", phone: "(51) 3311-7788", instagram: "@pizzarianapoli.poa",
    tags: ["Boa opção para delivery"],
    reviews: [
      { name: "Bruno", score: 10, comment: "Massa perfeita, borda crocante." },
      { name: "Larissa", score: 9, comment: "Entrega rápida e quentinha." },
    ] },
  { id: 4, name: "Café da Vó", category: "Cafeteria", bairro: "Bom Fim", score: 8.9, count: 67,
    address: "Rua João Alfredo, 320 - Bom Fim", phone: "(51) 3222-1156", instagram: "@cafedavopoa",
    reviews: [
      { name: "Juliana", score: 9, comment: "Bolo de fubá igual ao da minha avó." },
      { name: "Marcos", score: 9, comment: "Café coado impecável." },
    ] },
  { id: 5, name: "Boteco do Zé", category: "Comida brasileira", bairro: "Cidade Baixa", score: 8.4, count: 156,
    address: "Rua João Alfredo, 720 - Cidade Baixa", phone: "(51) 3212-9034", instagram: null,
    reviews: [
      { name: "Patrícia", score: 8, comment: "Feijoada de domingo é imbatível." },
      { name: "Thiago", score: 9, comment: "Porções generosas, preço justo." },
    ] },
  { id: 6, name: "Doceria Flor de Mel", category: "Doceria", bairro: "Petrópolis", score: 9.1, count: 52,
    address: "Av. Cel. Lucas de Oliveira, 95 - Petrópolis", phone: "(51) 3339-4471", instagram: "@flordemeldoceria",
    tags: ["Ambiente agradável"],
    reviews: [
      { name: "Vanessa", score: 9, comment: "Brigadeiro gourmet vale cada centavo." },
      { name: "Eduardo", score: 9, comment: "Embalagem linda para presente." },
    ] },
  { id: 7, name: "Sabor Oriental", category: "Japonês", bairro: "Centro", score: 7.8, count: 34,
    address: "Rua dos Andradas, 1230 - Centro", phone: "(51) 3224-6602", instagram: "@sabororientalpoa",
    reviews: [
      { name: "Renata", score: 8, comment: "Bom custo-benefício no rodízio." },
      { name: "Felipe", score: 7, comment: "Atendimento poderia ser mais ágil." },
    ] },
  { id: 8, name: "Hamburgueria Rústica", category: "Hamburgueria", bairro: "Auxiliadora", score: 8.2, count: 98,
    address: "Rua Mostardeiro, 220 - Auxiliadora", phone: "(51) 3029-8845", instagram: null,
    reviews: [
      { name: "Gabriela", score: 8, comment: "Pão brioche caseiro faz diferença." },
      { name: "Lucas", score: 8, comment: "Batata rústica é o ponto alto." },
    ] },
];

const seedPending = [
  { id: 101, name: "Espaço Verde Vegano", category: "Comida brasileira", bairro: "Bela Vista", scores: [9] },
  { id: 102, name: "Hot Dog do Marcão", category: "Hamburgueria", bairro: "Centro", scores: [8, 9] },
];

const seedLists = [
  { id: 201, name: "Rolê de sexta", members: ["Você", "Marina"], placeIds: [3, 1], visited: [1] },
  { id: 202, name: "Descobertas com a galera", members: ["Você", "Rafa", "Bia", "João"], placeIds: [2, 6, 5], visited: [] },
];
...
export default function App() {
  const [publicList, setPublicList] = useState(seedPublic);
  const [pendingList, setPendingList] = useState(seedPending);
  ...
  function openDetail(item) { setDetailItem(item); setScreen("detail"); }

  function openList(list) { setListDetail(list.id); setScreen("listDetail"); }

  function toggleVisited(placeId) {
    setLists((prev) => prev.map((l) => (l.id === listDetail
      ? { ...l, visited: l.visited.includes(placeId) ? l.visited.filter((x) => x !== placeId) : [...l.visited, placeId] }
      : l)));
  }

  function addPlaceToList(placeId) {
    setLists((prev) => prev.map((l) => (l.id === listDetail ? { ...l, placeIds: [...l.placeIds, placeId] } : l)));
    setListAddQuery("");
  }
  ...
  function createList() {
    ...
    const id = Date.now();
    setLists((p) => [...p, { id, name: newListName.trim(), members: ["Você"], placeIds: [], visited: [] }]);
    setNewListName("");
    setListDetail(id);
  ...
  function pickCategory(cat) {
    const draft = { id: Date.now(), name: draftName || rateQuery, category: cat, bairro: "sua indicação" };
    startRate("new", draft);
  }

  function submitScore(tags) {
    if (pickedScore == null) return;
    const chosenTags = tags || [];
    setMyCount((c) => c + 1);

    if (rateTarget.mode === "new") {
      const item = { ...rateTarget.data, scores: [pickedScore] };
      setPendingList((p) => [...p, item]);
      setConfirmInfo({ variant: "new", name: item.name, score: pickedScore, needed: MIN_COUNT - 1 });
    } else if (rateTarget.mode === "pending") {
      const updated = { ...rateTarget.data, scores: [...rateTarget.data.scores, pickedScore] };
      const avg = average(updated.scores);
      if (updated.scores.length >= MIN_COUNT && avg >= MIN_SCORE) {
        setPendingList((p) => p.filter((x) => x.id !== updated.id));
        setPublicList((p) => [...p, {
          id: updated.id, name: updated.name, category: updated.category, bairro: updated.bairro,
          score: Math.round(avg * 10) / 10, count: updated.scores.length, tags: chosenTags,
          reviews: [{ name: "Você", score: pickedScore, comment: "" }],
        }]);
        setConfirmInfo({ variant: "graduated", name: updated.name, score: Math.round(avg * 10) / 10 });
      } else {
        setPendingList((p) => p.map((x) => (x.id === updated.id ? updated : x)));
        setConfirmInfo({ variant: "pending", name: updated.name, score: pickedScore, needed: Math.max(0, MIN_COUNT - updated.scores.length) });
      }
    } else if (rateTarget.mode === "public") {
      const item = rateTarget.data;
      const newAvg = Math.round(((item.score * item.count + pickedScore) / (item.count + 1)) * 10) / 10;
      const updated = {
        ...item, score: newAvg, count: item.count + 1,
        tags: Array.from(new Set([...(item.tags || []), ...chosenTags])),
        reviews: [{ name: "Você", score: pickedScore, comment: "" }, ...item.reviews],
      };
      setPublicList((p) => p.map((x) => (x.id === item.id ? updated : x)));
  ...
  const filteredPending = pendingList
    .filter((e) => !activeCategory || e.category === activeCategory)
    .map((e) => ({ ...e, needed: Math.max(0, MIN_COUNT - e.scores.length) }));

  const rateMatches = rateQuery.length > 0
    ? [
        ...publicList.filter((e) => e.name.toLowerCase().includes(rateQuery.toLowerCase())).map((e) => ({ ...e, _mode: "public" })),
        ...pendingList.filter((e) => e.name.toLowerCase().includes(rateQuery.toLowerCase())).map((e) => ({ ...e, _mode: "pending" })),
      ]
    : [];
        ...
        <div style={{ padding: "0 16px 16px" }}>
          ...
          <div style={{ background: palette.surface, border: `1px solid ${palette.border}`, borderRadius: 14, padding: 12, marginBottom: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <span style={{ color: palette.text, fontSize: 12.5 }}>{item.address}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          {item.reviews.map((r, i) => (
            <div key={i} style={{ background: palette.surface, border: `1px solid ${palette.border}`, borderRadius: 14, padding: 12, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ color: palette.text, fontSize: 13, fontWeight: 600 }}>{r.name}</span>
    ...
    return (
      <div style={{ padding: "0 16px 16px" }}>
        ...
        {rateQuery !== "" && rateMatches.map((item) => (
          <EstablishmentRow
            key={item._mode + item.id}
            item={item._mode === "pending" ? { ...item, needed: Math.max(0, MIN_COUNT - item.scores.length) } : item}
    ...
    return (
      <div style={{ padding: "0 16px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "8px 0 20px" }}>
          ...
          <div>
            <p style={{ color: palette.text, fontSize: 15, fontWeight: 700, margin: 0 }}>Você</p>
  ...
  function renderLogin() {
    return (
      <div style={{ padding: "50px 24px 20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
        <button
          onClick={() => { setPendingAction(null); setScreen("home"); setActiveTab("home"); }}
          style={{ alignSelf: "flex-start", background: "none", border: "none", padding: 0, marginBottom: 34, cursor: "pointer" }}
        >
          <ChevronLeft size={22} color={palette.textMuted} />
        </button>
        <p style={{ fontFamily: "Georgia, serif", color: palette.text, fontSize: 24, fontWeight: 700, margin: "0 0 10px" }}>Aprovado</p>
        <p style={{ color: palette.textMuted, fontSize: 13, textAlign: "center", lineHeight: 1.6, margin: "0 0 36px", maxWidth: 260 }}>
          {loginMessage}
        </p>
        <button
          onClick={handleLogin}
          style={{ width: "100%", maxWidth: 280, padding: "13px 0", borderRadius: 14, background: "#F3E9DA", border: "none", color: "#1C1410", fontSize: 13.5, fontWeight: 700, cursor: "pointer", marginBottom: 10 }}
        >
          Continuar com o Google
        </button>
        <button
          onClick={handleLogin}
          style={{ width: "100%", maxWidth: 280, padding: "13px 0", borderRadius: 14, background: "#0B0805", border: `1px solid ${palette.border}`, color: palette.text, fontSize: 13.5, fontWeight: 700, cursor: "pointer" }}
        >
